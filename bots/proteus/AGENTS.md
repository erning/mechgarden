...
Current state: M3 complete (see `docs/roadmap.md`) — radar infinity lock, enemy
...
true surfing over the nearest waves discounted by bullet shadows from our own
tracked bullets, and a GF-bin gun trained by every-tick virtual waves. A
best-first PathSurfer exists but is disabled by default (it overfits the crude
danger model; see `docs/roadmap.md` M3) behind `Mover.MOVEMENT_ENGINE`. Later
