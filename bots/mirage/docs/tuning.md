...
环境说明：`Ronin` 与 `Kage` 内部自建 `java.util.Random()`（构造时按当前时间播种），
`Kitsune` 与 `Mirage` 走 Robocode 受控的 `Math.random()`。因此即便带 `--seed`，含
Ronin/Kage 的对战仍逐次随机、不可复现。`APS` 单样本标准差约 ±3–5（50 回合），
所以只有均值差 >~5、或多次重复一致时才算可靠信号。
...
```
JDK_JAVA_OPTIONS="-Dmirage.debug=true" python3 scripts/duel.py -r mirage -e kitsune -n 20 --show-output
```
...
| `mirage.profile` | `auto` \| `PURE_SURF` \| `KAGE_ORBIT` \| `WALL_SAFE` \| `RONIN_BASE` | 移动 profile：`auto`=按受血量 explore/exploit；指定名=整场强制该 profile；缺省=稳定的 `PURE_SURF`。 | `PURE_SURF` |
...
```
JDK_JAVA_OPTIONS="-Dmirage.profile=KAGE_ORBIT" python3 scripts/duel.py -r mirage -e kitsune -n 100
```
...
### 3.2 移动/火力的逐轮 explore/exploit 不帮忙，反而轻微扰动兄弟战

把移动 profile 选择器（`mirage.profile=auto`，逐轮 PURE_SURF↔KAGE_ORBIT 探索）和
火力选择器（`mirage.power=auto`，逐发 BALANCED↔ECONOMY 探索）打开后：

- 火力选择器对 Kitsune/Kage **始终选回 BALANCED**（对预测得准的对手，命中率几乎与
  功率无关，高功率单发伤害更高 → 净能量更高；低功率快子弹并未净赚）。
- 移动选择器对兄弟 surfer 对战**轻微变差**（Kitsune 选择器开 ~47.5 vs 关 50.4）：
  兄弟枪会学习我方访问模式，**稳定**的访问分布比不断切换的更难被锁定。
...
- `mirage.powerfloor` 0.1 / 0.5 / 1.2：对 Kitsune 表面看 0.5 更好（51），但 5×50 重复
  后回落到 48.7±3.5，与默认重叠——是噪声。Kage 反而偏好 1.2（低功率明显变差）。
  无单一地板对全部对手最优，故保持 `1.2`。
- `mirage.dck` 25 vs 40：5×50 重复后 Ronin 51.5→49.1、Kitsune 50.0→48.0，k=40 不更
  好（早期单样本的「Kitsune +5」是幸运样本）。保持 `25`。

### 3.4 兄弟直接对战是 ~50% 硬币战（在精确 MEA 之前）

`Mirage` 与 `Ronin`/`Kitsune`/`Kage` 同源、实力相当。纯参数微调（火力地板、
DC 的 k、移动/火力选择器）无法把这个胶着战稳定推向 >55%——差异都落在 ±3–5
的噪声带内。真正拉开差距需要新的**能力**，下一节就是一个。
...
- **枪侧用精确 MEA：净胜。** DC 的预测精度从 ~0.16 升到 ~0.19，实弹命中率从 ~8%
  升到 ~14%；打兄弟 surfer 直接拉开胜率：Ronin 50.7→57.6、Kitsune 49.2→58.3、
  Kage 50.4→52.8（DEALT 全面上升）。第三方 6 对手目录从 ~64% 升到 ~66%。
...
| Kitsune | ~58 | 稳定胜出（theory MEA 时 ~49）|
| Kage | ~53 | 胜出 |
...
枪侧精确 MEA 让 Mirages 在内部对战和第三方比赛中都稳定胜出 Ronin/Kitsune/Kage。
