...
class BulletShadows {
    ...
    /** Our gun fired [bullet] this tick: register it and cast its shadow on every
     ...
     * sweeps inside the annulus the front expands through that tick. Within a tick
     * the bullet runs B(s) = P + s·ΔB (s ∈ [0,1], P relative to the wave source),
     * the front spans [r, r + waveSpeed]; |B(s)|² is quadratic in s, so the
     * in-band sub-segment's endpoints come from the roots of |B(s)|² = r² and
     * |B(s)|² = (r + waveSpeed)², and the shadowed band is the GF interval.
     */
    private fun cast(
        shot: Shot,
        wave: EnemyWave,
    ...
    /** Candidate sub-tick positions for the in-band overlap: the segment ends plus
     * the roots (within the tick) of d² hitting either band boundary. */
