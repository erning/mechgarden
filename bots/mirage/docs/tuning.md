...
| `fp` | 本回合选中的火力 profile。 |
| `scorePower` | 得分火力门控状态：本轮使用 `auto` 或保持 `economy`，并显示历史存活回合、平均承伤和 backoff。 |
...
| `mirage.power` | `auto` \| `policy` \| `balanced` \| `economy` \| `pressure` \| `aggressive` | 火力 profile：`auto`=无视安全门控，强制按每发净能量 explore/exploit；`policy`=关闭得分火力门控；指定 profile 名=强制；缺省只在高生存、低承伤对局中自适应。 | 安全门控的 `ECONOMY` ↔ `BALANCED` |
...
### 3.2 移动自适应仍默认关闭；旧火力结论已失效

移动 profile 选择器（`mirage.profile=auto`）对同源 surfer 对战轻微变差：选择器开启时
约为 47.5 APS，关闭时为 50.4 APS。第三方 6 对手目录上，关闭选择器的稳定配置为
64.0 APS，高于开启时的 62.0 APS，也高于 Ronin 的 59.9 APS。因此移动选择器继续默认
关闭；稳定的访问分布比不断切换更难被同源枪锁定。

本节旧版还曾断言火力选择器“探索后始终选回 `BALANCED`”。Phase 12 发现该结论建立在
失效的射击结果归因上：Robocode 的子弹事件会重建 `Bullet` 对象，而
`PerShotRewards` 使用引用相等，导致任何实弹都没有完成 reward。旧 `auto` 实际一直
停留在第一个探索 profile，从未执行有效的 `BALANCED` ↔ `ECONOMY` 比较。修复和重新
验收后的默认行为见 Phase 12；本节只保留仍然成立的移动结论。
...
### Phase 12 · 高生存对局的得分火力门控

#### 问题与目录

以 2026-07-12 01:21:34 UTC 的 Mirage 0.9.4 RoboRumble 页面为快照，按 KNNPBI 从低到
高保留前 25 个配对，写入 `mirage-094-knnpbi-worst-25`。统一 35 回合基线为
77.58 APS、92.69% Survival、54.7 `dealt/r`、29.0 `taken/r`。

这 25 个对手并不都是生存问题。排除 Drifter、ISmart、Cyanide、BlackHole、Mimic 和
由 anti-ram 负责的 SuperRamFire 后，剩余 19 个对手的基线 Survival 为 99.25%，但
APS 只有 83.87，`dealt/r` 只有 58.2。它们写入
`mirage-094-score-pressure-19`，专门验收“已经活下来，但 bullet damage 与 bullet
bonus 不足”的问题。

`scripts/duel.py` 同时增加 `BONUS/r` 和 `OPP-BONUS/r`，并在 TSV 末尾追加
`bonus_per_round`、`enemy_bonus_per_round`。这样可以区分 APS 增益来自 Survival，还是
确实来自 bullet damage 和击杀奖励。

#### 两个 Bullet identity bug

Robocode 在 `BulletHitEvent`、`BulletMissedEvent` 和 `BulletHitBulletEvent` 中重建
`Bullet` 对象；`Bullet.equals()` 才通过引擎 bullet id 匹配原始子弹。Mirage 有两处
错误使用引用相等：

- `PerShotRewards` 从未结算命中、未命中或撞弹结果，`FirePowerSelector` 与
  `ShieldAimSelector` 实际上都没有学习；
- `BulletShadows` 无法删除已消失的我方子弹，死弹会继续向后续敌浪投射虚假 shadow。

两处都改为 `Bullet.equals()`，并用“同 `bullet id`、不同对象”的单元测试覆盖。只修
bug 的前 25 个配对复测为 77.61 APS，对 77.58 基线基本中性；它是必要的正确性修复，
但不能单独解决输出不足。

#### 为什么不全局开启自适应火力

修复后强制 `mirage.power=auto`，前 25 个配对从 77.48 升到 79.36 APS；20 个高生存对手
平均 `dealt/r +2.89`、`BONUS/r +0.63`、APS `+1.83`，Survival 只变化 −0.14 个
百分点。这证明按真实命中收益选择火力，可以把生存优势转换成伤害和击杀奖励。

但 Ronin、Pear、CassiusClay、SandboxDT、DrussGT、ScalarR 和 BeepBoop 的保护组从
41.98 降到 41.46 APS，Survival 从 42.45% 降到 39.18%。因此没有全局撤销 survival
policy 的 `ECONOMY`，而是增加无名称、跨回合的 `ScorePressureController`：

- 至少完成 8 回合；
- 历史 Survival 不低于 93%；
- 平均子弹承伤不高于 35；
- 死亡或单回合承伤不低于 45 后，退回 `ECONOMY` 3 回合；
- 本轮能量低于 30 后，剩余时间锁定 `ECONOMY`；
- anti-ram 近距高火力优先，不参与普通得分火力比较。

门控打开时，`BALANCED` 与 `ECONOMY` 按已解析加 pending 的发数交错探索，避免先连续
发射一种 profile，把枪的冷启动阶段误当成 profile 差异。门控打开前的 policy 子弹不
进入 reward，避免早期大量 `ECONOMY` 样本污染后续比较。回合结束时仍在飞行的自适应
子弹按 `-power` 结算；它们已经消耗能量但不会再得分，不能因为高能量子弹飞得较慢而
从统计中消失。显式
`mirage.power=policy` 可关闭门控，`mirage.power=auto` 可跳过门控做全局 A/B。

#### 35 回合验收

最终完整前 25 个配对为 78.18 APS、93.71% Survival、55.9 `dealt/r`、28.8
`taken/r`、10.8 `BONUS/r`。同口径 policy 对照为 77.48、93.03%、54.1、29.1、
10.3；候选分别变化 `+0.70 APS`、`+0.68` 个 Survival 百分点、`+1.8 dealt/r`、
`−0.3 taken/r` 和 `+0.5 BONUS/r`。

19 个得分目标又独立运行两组 35 回合 A/B：

| 运行 | Policy APS / Survival | 门控 APS / Survival | `dealt/r` 变化 | `BONUS/r` 变化 |
|---|---:|---:|---:|---:|
| 前 25 完整运行中的 19 个配对 | 82.49 / 97.74% | 84.13 / 98.95% | +2.43 | +0.60 |
| `--seed 942` 独立运行 | 83.21 / 98.95% | 83.56 / 98.35% | +1.40 | +0.30 |
| `--seed 945` 最终边界修复后 | 82.15 / 97.59% | 84.36 / 98.50% | +1.70 | +0.40 |
| **三组均值变化** |  | **+1.40 APS / +0.50 个百分点** | **+1.84** | **+0.43** |

三组 APS 都为正，且增益同时出现在 bullet damage 和 bullet bonus。DevilFISH 的调试
运行也直接观察到门控：前 8 回合保持约 1.40 平均弹力；第 9 回合进入 `auto` 后升到
1.69，第 11 回合为 2.01，单回合 `dealt`/击杀 bonus 达到 86/19；第 12 回合承伤
45.7 后立即安排 3 回合 backoff。说明该策略既能在安全对局中换取伤害，也能在风险
上升后回退。

强敌保护使用 Ronin、`expert` 和 `top`，共 7 个对手、每个 35 回合。`--seed 942`
下，policy 对照为 42.36 APS / 42.45% Survival，门控候选为 43.64 / 44.49%；
