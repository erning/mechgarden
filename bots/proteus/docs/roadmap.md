...
- **M7a 策略层（已完成）**：`strategy.Strategy` 只出标志位——anti-HOT（默
  认开启，被 |GF| > 0.15 的弹命中计数撤销，生效时 HOT 模型权重 ×2 且强
  制激活）、anti-ram（靠近速度 EMA > 5 或距离触发线 → 近垂直逃向移动，
  不冲浪）、anti-mirror（镜像点距离 EMA < 90px → walking-stick 宽松平
  滑）。验收（35 回合）：Grinnik 54.1%、BlackDeath 63.8%、Moebius 72.7%、
  stelo.Mirror 63.3%，全部 PWIN 100%；basic ~57.5（回归无损）。
- **M7b 残局收割（已完成）**：tryToDisable（我方 > 3 能量、敌 < 95、无来
  波、敌停火 > 60 tick → 打「留 0.011」威力）；敌 < 0.5 停火等撞击；敌
  < 0.03 → 撞击收割（ram bonus）。Grinnik 复测 68.7%（bonus/r 8.9）。
- **M7c 子弹护盾（已完成，全在线版）**：`shield.AimPredictors` 五个候选预
  测器（正直/线性/圆形/晚拍/外推）在线回测偏差；「样本 ≥ 10、std <
  0.02、均值 < 0.03 且敌命中率 < 12%」才进入护盾（无静态名单，宁缺勿
  滥）；拦截计划解算（威力 × 角度）组合，交点侧移 0.1px 防共线退化；退
  出条件（停火 300 tick、受伤 150、预测失稳）触发后清空命中率统计。实
  测：Grinnik 75.98%（护盾接管，taken/r 30.1）、BlackDeath 66.2%；对
  RaikoNano / FloodMini 正确保持不进入（枪不够可预测）。

原定验收：`roborumble-100` catalog 全量 APS 目标，M7c 完成后定标。

## M8 离线训练闭环（已完成基础设施，首轮权重打平）

范围（已交付）：`diag.Dataset`（内存缓冲 + 战斗结束落盘，规避引擎 5 流限
制与 200KB 配额）；`scripts/collect_dataset.py`（per-pairing 最小 robots
目录采集，收割 .data 到 `.cache/proteus-datasets/`）；`src/train` source
set 与 `:bots:proteus:trainEmbedding` 任务（批累积梯度 SGD、乘性更新、
权重总和归一、近邻集合每轮重算）；`docs/training.md` 流程文档。

踩坑记录：RobocodeFileOutputStream 必须用引擎规定的流类型；数据配额按目
录内已有文件累计；大 robots 目录使个别 jar 解析为 invalid robot；梯度公
式写错会系统性恶化（正确推导 dL/dlogit_j = α_j(1 − B_j/D)）。

首轮结果：basic+classic 5 对手 15k 样本、24 epoch，损失 1.4267 → 1.3897；
回读后 basic/classic 与手调权重噪声内打平（权重已保留在 `Features.WEIGHTS`）。
原定「与手工权重 A/B 有稳定正收益」未达成——方向：更多对手与样本、按对
手分训专家模型、危险模型权重离线化。
...
| 2026-07-21 | M7a | Grinnik/BlackDeath/Moebius/Mirror | 35 | 54~73% | 100% | 分类器专项 |
| 2026-07-21 | M7b | Grinnik | 35 | 68.7% | 100% | 残局收割，bonus/r 8.9 |
| 2026-07-21 | M7c | Grinnik | 35 | 76.0% | 100% | 护盾接管，taken/r 30.1 |
| 2026-07-21 | M7c | BlackDeath | 35 | 66.2% | 100% | 护盾+反 ram |
| 2026-07-21 | M8 | basic | 35×2 | 53.4/58.1 | 100% | 首轮学习嵌入，与手调打平 |
| 2026-07-21 | M8 | classic | 35 | 39.7% | 0% | 同上 |
